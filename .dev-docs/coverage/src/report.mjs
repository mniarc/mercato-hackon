    ...
    for (const feature of domain.features) {
      ...
      for (const child of feature.children) {
        ...
        for (const criterion of child.criteria) {
          ...
          if (criterion.trialReadiness) {
            lines.push(`        Readiness: ${criterion.trialReadiness.label} [Action: ${criterion.trialReadiness.actionRole}]`)
            if (criterion.trialReadiness.reason && !criterion.missing.includes(criterion.trialReadiness.reason) && !criterion.externalDecision.includes(criterion.trialReadiness.reason)) {
              lines.push(`        Readiness note: ${criterion.trialReadiness.reason}`)
            }
          }
...
function criterionHtml(criterion, appRoot, outputPath) {
  ...
  const readiness = criterion.trialReadiness
    ? `<span class="badge readiness-${criterion.trialReadiness.kind}" title="${htmlEscape(criterion.trialReadiness.reason)}">${htmlEscape(criterion.trialReadiness.actionRole)}: ${htmlEscape(criterion.trialReadiness.label)}</span>`
    : ''
  ...
  return `<li class="criterion"><div class="criterion-title"><strong>${htmlEscape(criterion.id)}</strong><span class="badge state-${criterion.implementation}" title="Manual code implementation assessment">Code: ${stateLabel(criterion.implementation)}</span>${readiness}${proof}</div><p>${htmlEscape(criterion.text)}</p>${evidence ? `<div class="evidence"><b>Recorded evidence and notes</b><ul>${evidence}</ul></div>` : ''}${remaining}${criterion.externalDecision.length ? `<div class="callout decision"><b>External decision</b><ul>${criterion.externalDecision.map((item) => `<li>${htmlEscape(item)}</li>`).join('')}</ul></div>` : ''}${criterion.assessmentSource ? `<small>Assessment: ${fileLink(criterion.assessmentSource, criterion.assessmentSource, appRoot, outputPath)}</small>` : ''}</li>`
}

function storyHtml(story, appRoot, outputPath) {
  const readinessKinds = [...new Set(story.criteria.map((c) => c.trialReadiness?.kind).filter(Boolean))].join(' ')
  const primaryBlocker = story.primaryBlocker ?? ''
  const blockerBadge = primaryBlocker && !['trial_proved', 'unassessed'].includes(primaryBlocker)
    ? `<span class="badge readiness-${primaryBlocker}" title="Primary blocker: ${htmlEscape(trialReadinessLabels[primaryBlocker])}">${htmlEscape(trialReadinessActionRoles[primaryBlocker])}</span>`
    : ''
  const searchable = [story.id, story.title, story.userStory, story.featureTitle, story.domain, story.scope, ...story.taskEvidence.flatMap((task) => [task.id, task.title, task.stateLabel]), ...story.criteria.flatMap((criterion) => [criterion.id, criterion.text, criterion.trialReadiness?.label, criterion.trialReadiness?.actionRole, ...criterion.missing, ...criterion.externalDecision, ...criterion.evidence.flatMap((item) => [item.path, item.note])])].filter(Boolean).join(' ').toLowerCase()
