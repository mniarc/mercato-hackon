...
function criterionHtml(criterion, appRoot, outputPath) {
  const proof = verificationKinds.map((kind) => `<span class="proof proof-${criterion.verification[kind]}">${{ focused: 'Focused', nativeApp: 'Native app / fixture', liveModel: 'Live model' }[kind]}: ${criterion.verification[kind]}</span>`).join('')
  ...
  return `<li class="criterion"><div class="criterion-title"><strong>${htmlEscape(criterion.id)}</strong><span class="badge state-${criterion.implementation}">${stateLabel(criterion.implementation)}</span>${proof}</div><p>${htmlEscape(criterion.text)}</p>${evidence ? `<div class="evidence"><b>Evidence</b><ul>${evidence}</ul></div>` : ''}${criterion.missing.length ? `<div class="callout missing"><b>Missing</b><ul>${criterion.missing.map((item) => `<li>${htmlEscape(item)}</li>`).join('')}</ul></div>` : ''}${criterion.externalDecision.length ? `<div class="callout decision"><b>External decision</b><ul>${criterion.externalDecision.map((item) => `<li>${htmlEscape(item)}</li>`).join('')}</ul></div>` : ''}${criterion.assessmentSource ? `<small>Assessment: ${fileLink(criterion.assessmentSource, criterion.assessmentSource, appRoot, outputPath)}</small>` : ''}</li>`
}
...
export function renderHtmlReport(report, { appRoot = defaultAppRoot, outputPath = path.join(appRoot, '.dev-docs', 'coverage', 'report.html') } = {}) {
  ...
  const manualInputs = report.assessmentSources.map((source) => fileLink(source, source, appRoot, outputPath)).join(', ') || 'none'
...
export async function refreshOutputs(report, appRoot = defaultAppRoot) {
  const htmlPath = path.join(appRoot, '.dev-docs', 'coverage', 'report.html')
