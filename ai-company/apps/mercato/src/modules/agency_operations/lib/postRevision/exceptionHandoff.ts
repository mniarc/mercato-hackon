...
import { POST_REVISION_RESULT_KEY, postRevisionActivityResultSchema } from './contracts'
...
export function createPostRevisionResearchExceptionHandoff(container: AppContainer) {
  return async (_input: unknown, rawContext: unknown) => {
    ...
    if (!source) throw new Error('[internal] Post revision exception requires the originating native workflow')
    const saved = z.object({ result: postRevisionActivityResultSchema }).parse(source.context[POST_REVISION_RESULT_KEY]).result
    ...
    if (!submission) throw new Error('[internal] Post revision exception is outside the originating case submission')
    ...
    if (saved.submissionId !== submission.id || !original || original.kind !== 'message' || original.post.versionId !== saved.previousPostVersionId) {
      throw new Error('[internal] Post revision exception does not belong to the original post response')
    }
    ...
    if (!agencyCase) throw new Error('[internal] Post revision exception case is outside the submission scope')
    ...
    if (!exception || exception.orderRef !== agencyCase.id || exception.versionId !== saved.escalationVersionId
      ...
      throw new Error('[internal] Post revision exception must be the current open blocked version')
    }
    if (!saved.documentVersionIds.includes(exception.versionId) || !saved.taskRunIds.includes(exception.taskRunId)) {
      throw new Error('[internal] Post revision exception is not an output of the saved revision result')
    }
