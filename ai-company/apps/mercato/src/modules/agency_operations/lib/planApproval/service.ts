...
import { AGENCY_RESEARCH_SERVICE, type AgencyResearchService, type AcceptPlanInput } from '@/modules/agency_research/lib/contracts'
...
import { PLAN_REVIEW_CONTEXT_KEY, PLAN_REVIEW_WORKFLOW_ID, PLAN_RESPONSE_CONTEXT_KEY, planReviewInvitationSchema, planReviewRequestSchema } from '../planReview/contracts'
...
export function createPlanApproval(container: AppContainer) {
  async function load(submission: AgencyClientSubmission, interpretation: unknown): Promise<AcceptPlanInput['request'] | null> {
    ...
    const response = preserved.data.original.planReviewResponse
    if (!response || response.kind !== 'approval' || response.approvePlan !== true || !response.selectedTopicId) return null
    ...
    }, interpretation, ['plan_topic_decision'])
    if (candidate.disposition?.kind !== 'approve' || candidate.disposition.targetStepId !== 'plan_topic_decision') return null
    const original = planReviewRequestSchema.safeParse(Object.fromEntries(Object.entries(response).filter(([key]) => key !== 'taskId')))
    if (!original.success || preserved.data.original.documentVersionReference !== response.plan.versionId
      || submission.eventId !== planReviewEventId(response.taskId, response.externalEventId)
    ...
    if (!task || !isDeepStrictEqual(record(task.formData)[PLAN_RESPONSE_CONTEXT_KEY], original.data)) return null
    const invitationWorkflow = await findOneWithDecryption(em, WorkflowInstance, {
      id: task.workflowInstanceId, workflowId: PLAN_REVIEW_WORKFLOW_ID, ...scope, deletedAt: null,
    }, undefined, scope)
    const invitation = planReviewInvitationSchema.safeParse(invitationWorkflow?.context[PLAN_REVIEW_CONTEXT_KEY])
    if (!invitation.success || invitation.data.caseId !== submission.caseId || invitation.data.review.caseId !== submission.caseId
      ...
      || invitation.data.review.plan.documentId !== response.plan.documentId || invitation.data.review.plan.versionId !== response.plan.versionId) return null
    ...
    return { orderRef: submission.caseId, documentId: response.plan.documentId, versionId: response.plan.versionId,
      approvePlan: true, selectedTopicId: response.selectedTopicId, customerUserId: submission.submittedByCustomerUserId,
  ...
  return {
    ...
    async accept(submission: AgencyClientSubmission, interpretation: unknown) {
      ...
      if (!request) throw new Error('[internal] Plan approval requires the exact authorized plan/topic original and saved native triage decision')
      const scope = { tenantId: submission.tenantId, organizationId: submission.organizationId }
      const em = container.resolve<EntityManager>('em')
      ...
      if (!workflow) throw new Error('[internal] Plan approval requires the native client submission workflow')
      const userId = await resolveWorkflowPrincipalUserId(em, workflow)
      if (!userId) throw new Error('[internal] Plan approval requires the native workflow principal')
      return container.resolve<AgencyResearchService>(AGENCY_RESEARCH_SERVICE).acceptPlan({ context: { ...scope, userId }, request })
    },
  }
}
