...
import { PLAN_REVIEW_CONTEXT_KEY as INVITATION, PLAN_RESPONSE_CONTEXT_KEY as RESPONSE,
  PLAN_REVIEW_WORKFLOW_ID as WORKFLOW, planReviewInvitationSchema, planReviewRequestSchema, planReviewSchema, type PlanReviewService, type PlanReviewRequest } from './contracts'
import { planReviewEligible, renderPlanReview } from './review'
...
export const planReviewEventId = (taskId: string, eventId: string) => `plan-review:${taskId}:${createHash('sha256').update(eventId).digest('hex')}`
function responseAvailable(review: z.infer<typeof planReviewSchema>, input?: PlanReviewRequest) {
  if (!review.plan.isCurrent || !['approved', 'ready_for_review'].includes(review.plan.status)) return false
  return input?.kind !== 'approval' || review.topics.some((topic) => topic.topicId === input.selectedTopicId)
}

export function createPlanReviewService(container: AppContainer): PlanReviewService {
  ...
  async function loadTask(taskId: string, scope: Scope) {
    ...
    if (!instance) missing()
    const parsed = planReviewInvitationSchema.safeParse(instance.context[INVITATION])
    ...
    if (!agencyCase || invitation.review.caseId !== agencyCase.id || invitation.review.plan.caseId !== agencyCase.id
  ...
  async function currentReview(loaded: Awaited<ReturnType<typeof loadTask>>, scope: Scope) {
    ...
    const current = await research.getPlanReview(scope, { orderRef: loaded.agencyCase.id, planVersionId: snapshot.plan.versionId })
    const eligible = planReviewEligible(current, loaded.agencyCase.id)
    const matches = eligible && current.plan.documentId === snapshot.plan.documentId && current.plan.versionId === snapshot.plan.versionId
    const { acceptanceReceipt: _snapshotReceipt, ...document } = snapshot.plan
    const { selectedTopicId: _snapshotSelection, ...review } = snapshot
    return { ...review, plan: { ...document, isCurrent: Boolean(matches),
    ...
    }, ...(matches && current.receipt ? { selectedTopicId: current.receipt.selectedTopicId } : {}) }
  }
  function matchesPlan(input: PlanReviewRequest, loaded: Awaited<ReturnType<typeof loadTask>>) {
    return input.plan.documentId === loaded.invitation.review.plan.documentId && input.plan.versionId === loaded.invitation.review.plan.versionId
  }
  async function existingReceipt(task: UserTask, loaded: Awaited<ReturnType<typeof loadTask>>, response: PlanReviewRequest, scope: Scope) {
    const submission = await findOneWithDecryption(em, AgencyClientSubmission, {
      ...
      eventId: planReviewEventId(task.id, response.externalEventId), deletedAt: null,
    }, undefined, scope)
    return submission && submission.submittedByCustomerUserId === task.assignedTo
      && isDeepStrictEqual(submission.original.planReviewResponse, { taskId: task.id, ...response }) ? submission : null
  }
  return {
    async invite(rawInput) {
      const input = z.object({ caseId: z.uuid(), planVersionId: z.uuid(), tenantId: z.uuid(), organizationId: z.uuid(), userId: z.uuid() }).parse(rawInput)
      ...
      const started = await em.transactional(async (tx) => {
        ...
        const current = await research.getPlanReview(scope, { orderRef: agencyCase.id, planVersionId: input.planVersionId })
        if (!planReviewEligible(current, agencyCase.id)) conflict()
        const suffix = current.receipt ? `:after:${current.receipt.source.submissionId}` : ''
        const correlationKey = `agency-plan:${agencyCase.id}:${input.planVersionId}${suffix}`
        ...
        if (!definition?.enabled || definition.metadata?.generatedBy?.module !== 'agency_operations' || definition.metadata.generatedBy.ownerId !== 'plan_review') conflict()
        const review = await renderPlanReview(current, agencyCase.id)
    ...
    async respond(auth, taskId, rawInput) {
      ...
      if (task.assignedTo !== auth.sub || !matchesPlan(input, loaded)) missing()
      if (task.status === 'COMPLETED') {
        const original = planReviewRequestSchema.safeParse(record(task.formData)[RESPONSE])
    ...
    async receiveResponse(rawInput, rawContext) {
      ...
      const input = planReviewRequestSchema.parse(record(rawInput).response)
      const original = planReviewRequestSchema.parse(record(task.formData)[RESPONSE])
      if (task.completedBy !== loaded.invitation.customerUserId || !isDeepStrictEqual(input, original) || !matchesPlan(original, loaded)) missing()
      ...
      const result = await container.resolve<ClientSubmissionService>(CLIENT_SUBMISSION_SERVICE).submit({
        ...
        eventId: planReviewEventId(task.id, original.externalEventId),
        text: original.kind === 'message' ? original.body : `Client explicitly approves plan ${original.plan.versionId} and selects topic ${original.selectedTopicId}.`,
        documentVersionReference: original.plan.versionId,
        planReviewResponse: { taskId: task.id, ...original },
      })
