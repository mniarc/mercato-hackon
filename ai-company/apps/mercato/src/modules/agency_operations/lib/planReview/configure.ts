...
import { PLAN_REVIEW_WORKFLOW_ID } from './contracts'
import { planReviewWorkflowDefinition } from './workflow'

export async function configurePlanReviewWorkflow(container: AppContainer, rawInput: unknown) {
  ...
  const existing = await authoring.findOwnedDefinition(em, { workflowId: PLAN_REVIEW_WORKFLOW_ID, ...scope })
  if (existing) {
    if (existing.metadata?.generatedBy?.module !== 'agency_operations' || existing.metadata.generatedBy.ownerId !== 'plan_review') throw new CrudHttpError(409, { error: 'api.errors.conflict' })
    return { workflowId: existing.workflowId, version: existing.version }
  }
  const created = await authoring.upsertOwnedDefinition(em, {
    ...scope, ownerModule: 'agency_operations', ownerId: 'plan_review', workflowId: PLAN_REVIEW_WORKFLOW_ID,
    workflowName: 'Plan approval and topic selection', definition: planReviewWorkflowDefinition, actorUserId: input.userId, grantedFeatures: [],
  })
