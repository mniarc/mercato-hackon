...
import { createTovWorkflowActivity, AGENCY_TOV_RESULT_CONTEXT_KEY } from '../../tovProcess'
...
const ids = { tenantId: uuid(1), organizationId: uuid(2), userId: uuid(3), caseId: uuid(4), customerEntityId: uuid(5),
  workflowId: uuid(6), intakeId: uuid(7), attachmentId: uuid(8), stepId: uuid(9), runId: uuid(10), versionId: uuid(11), agentRunId: uuid(12) }
...
it('reads the exact staff-owned corpus and persists only the specialist result references', async () => {
  ...
  jest.mocked(findOneWithDecryption).mockResolvedValue({ id: ids.caseId, customerEntityId: ids.customerEntityId } as never)
  ...
  const context = { userId: ids.userId, stepInstanceId: ids.stepId, workflowInstance: { id: ids.workflowId,
