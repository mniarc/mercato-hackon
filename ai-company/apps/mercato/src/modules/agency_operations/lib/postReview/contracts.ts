...
import { publicationTargetSchema } from '@/modules/agency_research/lib/contracts'
...
export const postReviewSchema = z.object({ caseId: z.uuid(), post: documentReviewSchema,
  publicationTarget: publicationTargetSchema.optional(),
  publicationConsentReceipt: z.object({ consentedAt: z.iso.datetime() }).optional(),
})
...
export const postReviewRequestSchema = z.object({
  ...
  publicationConsent: z.object({ configVersionId: z.uuid(), consent: z.literal(true) }).strict().optional(),
  ...
  if (input.kind === 'message' && (!input.body?.trim() || input.approveContent !== undefined || input.publicationConsent !== undefined)) {
