...
const translations: Record<string, string> = {
  ...
  'agencyOperations.cases.researchLedger.title': 'Persisted research work',
  'agencyOperations.cases.researchLedger.empty': 'No research attempts recorded for this case.',
...
test('the detail composes public case and workflow APIs into persisted evidence', async () => {
  apiCallMock.mockImplementation(async (input) => {
    ...
    if (url === '/api/agency_research/task-runs?order_ref=case-1') {
      return successfulCall({ orderRef: 'case-1', totalPln: 0, sources: 0, taskRuns: [], documents: [] })
    }
  ...
  expect(await screen.findByText('No research attempts recorded for this case.')).toBeTruthy()
  expect(screen.getByRole('heading', { name: 'Persisted research work' })).toBeTruthy()
  ...
  expect(new Set(apiCallMock.mock.calls.map(([input]) => String(input)))).toEqual(new Set([
    ...
    '/api/agency_research/task-runs?order_ref=case-1',
...
test('a missing case renders the dedicated not-found state', async () => {
  ...
  expect(new Set(apiCallMock.mock.calls.map(([input]) => String(input)))).toEqual(new Set([
    '/api/agency_operations/cases?id=missing-case&pageSize=1',
  ]))
})
