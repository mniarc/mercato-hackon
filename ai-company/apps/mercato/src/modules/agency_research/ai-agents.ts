...
import {
  ...
  RESEARCH_QA_AGENT_ID,
} from './lib/agentIds'

export * from './lib/agentIds'

// The audit-and-research agents (P3, steps 3.2 → 3.7) are RESEARCHERS with NO
// tools, driven map → reduce by `lib/research/pipeline.ts`. Each reads one bounded
// input (one page, or the fact bank as ids + short claims) and returns one SECTION
// of a document — never a whole document, never the envelope. Ids are minted by
// code; every quote is checked verbatim against the stored page by the gate.
//
// Model tiers (STD-LIMITY cost control): extraction and QA on the cheap model,
// syntheses on the strong one. Overridable per tier through the environment;
// the platform's model factory still honours `OM_AI_AGENCY_RESEARCH_MODEL` and a
// caller override above these defaults.

const MODEL_EXTRACT = process.env.OM_AGENCY_RESEARCH_MODEL_EXTRACT ?? 'openrouter/anthropic/claude-haiku-4.5'
const MODEL_SYNTHESIS = process.env.OM_AGENCY_RESEARCH_MODEL_SYNTHESIS ?? 'openrouter/anthropic/claude-sonnet-5'
const MODEL_QA = process.env.OM_AGENCY_RESEARCH_MODEL_QA ?? MODEL_EXTRACT
...
export const aiAgents: AiAgentDefinition[] = [
  ...
  defineAgent({
    ...
    instructions: [
      ...
      'list is correct after checking — never invent a conflict to fill the list.',
      SHARED_RULES,
    ].join(' '),
