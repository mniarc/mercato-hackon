...
import { publicationConsentRecordSchema, publicationConsentSourceRef, recordPublicationConsentInputSchema, samePublicationDestination, type PublicationConsentResult } from './contracts'
...
export async function recordPublicationConsent(manager: EntityManager, rawInput: unknown): Promise<PublicationConsentResult> {
  ...
  return manager.transactional<PublicationConsentResult>(async (em) => {
    ...
    const source = 'kind' in request.source ? request.source : { kind: 'agency_publication_consent' as const, ...request.source }
    const records = Array.isArray(version.approvalRecords) ? version.approvalRecords : []
    const previous = records.map((raw) => publicationConsentRecordSchema.safeParse(raw)).find((saved) => saved.success && publicationConsentSourceRef(saved.data.source) === publicationConsentSourceRef(source))
    if (previous?.success) {
      ...
      if (saved.person !== request.customerUserId || saved.at !== request.decidedAt || saved.documentId !== document.id || saved.documentVersionId !== version.id
        || !isDeepStrictEqual(saved.source, source) || !isDeepStrictEqual(saved.destination, request.destination)
        || ('expectedContentHash' in request && request.expectedContentHash !== saved.contentHash)) throw new CrudHttpError(409, { error: 'api.errors.conflict' })
    ...
    if (accepted.status !== 'ready' || !accepted.receipt || accepted.receipt.person !== request.customerUserId
      || (source.kind === 'native_publication_consent_task'
    ...
    if ('expectedContentHash' in request && request.expectedContentHash !== contentHashOf(post)) return { status: 'not_ready', reason: 'content_changed' }
