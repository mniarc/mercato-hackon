...
export const sourcesAgents: AiAgentDefinition[] = [
  ...
  defineAgent({
    ...
    instructions: promptFor(RESEARCH_PAGE_EXTRACTOR_AGENT_ID, [
      ...
      'and the observed function of the fragment. When `page.publisher` is a person’s name (a',
      'founder or spokesperson of the brand — their own post, blog or an interview with them), the',
      'language samples are THAT person’s voice (name the person in `situation`) and the facts',
      'are the brand’s `first_party_claim` unless a third party states them; a journalist’s',
      'framing in an interview is not the person’s wording. `audience_signals`: only when the page',
  ...
  defineAgent({
    ...
    instructions: promptFor(RESEARCH_CONFLICT_FINDER_AGENT_ID, [
      ...
      'brief does not repeat an unbacked number. Social post vs website about the offer, scope',
      'or positioning: when the post is older (`published_at`, or its `limitation` marks it dated)',
      'the conflict is `possibly_outdated` with the post as the dated side and the question "does',
