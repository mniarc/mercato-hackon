...
const BRIEF_RULES = [
  ...
  'reads as plain sentences a client can act on — evidence ids never appear inside prose,',
...
export const briefAgents: AiAgentDefinition[] = [
  ...
  defineAgent({
    ...
    instructions: [
      ...
      'with `fact_ids` — never a good one against a bad one; `sample_ids` cited). When',
      '`people` is non-empty, `desired_traits` describes how the named person actually writes',
      '(from THEIR `sample_ids`, name them in the trait, e.g. "głos Rafała Mudy: …"), the',
  ...
  defineAgent({
    ...
    instructions: [
      ...
      'functionality status — a visible address is not a working contact — `owner` = the person',
      'from `people` who publishes on the serviced channel or is the order\'s contact, written as',
      'a proposal ("propozycja: …"), null only when `people` is empty; `limits`), `success_and_limits` (directional goal, 1–3 `measurement_proposals` with',
