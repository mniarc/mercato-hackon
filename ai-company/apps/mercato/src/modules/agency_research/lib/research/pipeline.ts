...
import { GateError, gateConflicts, gateCoverage, gatePageExtraction, gateProofCards, gateSeeds, planCapacity, type GateIssue } from './gate'
...
import { fingerprint, mapWithConcurrency, quoteOffset } from './util'
 ...
 * content bank, conflicts and coverage over ids — never over page text. Every
 * call: budget check → cache or agent → zod → gate → (re-request ≤ N) → record.
 *
...
const DEFAULT_EXTRACT_TIMEOUT_MS = 3 * 60_000
const DEFAULT_SYNTHESIS_TIMEOUT_MS = 8 * 60_000
const EXPECTED_OUTPUT_TOKENS = { extract: 2_500, synthesis: 6_000, qa: 1_500 } as const
...
function factBankInput(order: OrderFacts, lang: OutputLanguage, sources: Source[], facts: Fact[], samples: LanguageSample[], signals: AudienceSignal[]): FactBankInput {
  return {
    ...
    outputLanguage: lang,
    sources: sources.filter((s) => s.access !== 'unavailable').map((s) => ({ source_id: s.source_id, publisher: s.publisher, kind: s.kind, url: s.url_or_file, access: s.access, retrieved_at: s.retrieved_at })),
...
export async function runSourcesStep(opts: Step32Options): Promise<Step32Result> {
  ...
  const conflicts: Conflict[] = conflictsOut.value.conflicts.map((conflict, index) => ({
    ...
    dates: [...new Set(conflict.fact_ids.flatMap((id) => facts.find((f) => f.fact_id === id)?.source_ids ?? []).map((sid) => sources.find((s) => s.source_id === sid)?.retrieved_at ?? '').filter(Boolean))],
