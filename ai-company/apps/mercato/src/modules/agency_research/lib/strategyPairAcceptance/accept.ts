...
export async function acceptStrategyPair(container: AppContainer, rawInput: unknown): Promise<StrategyPairAcceptanceReceipt> {
  ...
  return container.resolve<EntityManager>('em').fork().transactional(async (em) => {
    ...
    const previous = pairDocumentKinds.flatMap((kind) => (rows[kind].version.approvalRecords as unknown[])
      ...
      .filter((parsed) => parsed.success && parsed.data.source.submissionId === source.submissionId)
    ...
    if (previous.length) {
      ...
      for (const { kind, record } of previous) {
        if (!record || !request.approvedDocuments.includes(kind) || record.scope !== kind
          || record.person !== request.customerUserId || record.documentVersionId !== rows[kind].version.id
          || record.version !== versionLabel(rows[kind].version.versionNo) || !isDeepStrictEqual(record.source, source)
    ...
    const checked = await readEligiblePair(em, scope, { orderRef: request.orderRef,
      strategyVersionId: request.pair.strategy.versionId, tovVersionId: request.pair.tov.versionId })
    ...
    for (const kind of pairDocumentKinds) {
      ...
      if (document.currentVersionId !== version.id || checked.pair[kind].documentId !== document.id) conflict()
      if ((document.status === 'approved' || version.status === 'approved') && !matchingPairAcceptance(version.approvalRecords, kind, checked.pair)) conflict()
    }
    const at = new Date().toISOString()
    const records = request.approvedDocuments.map((kind) => strategyPairAcceptanceRecordSchema.parse({
      person: request.customerUserId, at, scope: kind, version: versionLabel(rows[kind].version.versionNo),
      documentVersionId: rows[kind].version.id, briefVersionId: checked.pair.brief.versionId,
