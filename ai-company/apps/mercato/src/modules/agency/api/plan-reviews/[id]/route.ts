...
import { PLAN_REVIEW_SERVICE, planReviewReadSchema, planReviewRequestSchema, planReviewReceiptSchema, type PlanReviewService } from '@/modules/agency_operations/lib/planReview/contracts'
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const result = await container.resolve<PlanReviewService>(PLAN_REVIEW_SERVICE).read(auth, taskId)
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const input = planReviewRequestSchema.safeParse(raw)
    ...
    const guarded = planReviewRequestSchema.safeParse({ ...input.data, ...guard.modifiedPayload })
    if (!guarded.success) throw new CrudHttpError(400, { error: 'api.errors.badRequest' })
    const result = await container.resolve<PlanReviewService>(PLAN_REVIEW_SERVICE).respond(auth, taskId, guarded.data)
    await guard.runAfterSuccess()
    return Response.json(result, { status: result.replayed ? 200 : 201, headers: clientCaseResponseHeaders })
  } catch (error) { return error instanceof Response ? error : clientCaseErrorResponse(error) }
}
const errors = [{ status: 401, description: 'Customer authentication required' }, { status: 403, description: 'Native portal task permission required' }, { status: 404, description: 'Review task or plan not visible to this customer' }]
export const openApi: OpenApiRouteDoc = {
  tag: 'Agency', pathParams,
  methods: {
    GET: { summary: 'Read the exact invited plan, topics and recommendation', responses: [{ status: 200, schema: planReviewReadSchema }], errors },
    POST: { summary: 'Receive an original plan response through shared intake; not document acceptance',
      requestBody: { contentType: 'application/json', schema: planReviewRequestSchema },
      responses: [200, 201].map((status) => ({ status, schema: planReviewReceiptSchema })),
      errors: [...errors, { status: 400, description: 'Invalid plan response' }, { status: 409, description: 'Stale plan, invalid topic or QA or response conflict' }, { status: 413, description: 'Request exceeds 100 KB' }],
    },
  },
}
