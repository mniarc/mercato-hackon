...
1. Sprawa wskazuje klienta, kupiony produkt, zamówienie i instancję ustalonego STD-PROCES.
2. Każde zadanie pokazuje dział, wykonawcę agentowego, stan, wejście, wynik oraz zależności.
3. Oczekiwanie na klienta, blokada zależności i wyjątek pracownika mają czytelny powód oraz właściwego adresata działania.
4. Dyspozycja G i powrót z E są powiązane z zadaniem i historią zamówienia.
5. Widok prezentuje rzeczywiście zapisane wykonania; funkcjonalność agencji jest częścią budowanego modułu.
6. Po spełnieniu warunków zadania agentów mogą ruszać niezależnie od godzin pracy ludzi; oczekiwanie ma konkretny powód: klient, zależność albo wyjątek E. Nie stanowi to obietnicy konkretnego czasu realizacji.
